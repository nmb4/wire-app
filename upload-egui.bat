...
if not exist "%ZIP_PATH%" (
    ...
    echo Run package-egui.bat first.
    exit /b 1
)
...
set "RESPONSE_FILE=%TEMP%\callme-upload-%RANDOM%-%RANDOM%.json"
...
set "CALLME_UPLOAD_RESPONSE=%RESPONSE_FILE%"
for /f "usebackq delims=" %%I in (`powershell -NoProfile -Command "$response = Get-Content -Raw $env:CALLME_UPLOAD_RESPONSE | ConvertFrom-Json; if ($response.id) { $response.id } elseif ($response.file.id) { $response.file.id } else { exit 1 }"`) do set "FILE_ID=%%I"
