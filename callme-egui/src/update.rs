...
mod tests {
    ...
    fn parses_release_filename() {
        let file = FileEntry {
            id: "abc".into(),
            original_name: "callme-egui-v1.12.3.zip".into(),
            sha256: "hash".into(),
        };
    ...
    fn ignores_non_release_files() {
        let file = FileEntry {
            id: "abc".into(),
            original_name: "callme-egui.zip".into(),
            sha256: "hash".into(),
        };
    ...
    fn selects_highest_version_newer_than_current() {
        ...
        let release = latest_newer_release(
            vec![
                file("callme-egui-v0.1.2.zip"),
                file("callme-egui-v0.2.0.zip"),
                file("unrelated.zip"),
            ],
            [0, 1, 2],
        )
        ...
        assert!(latest_newer_release(vec![file("callme-egui-v0.1.2.zip")], [0, 1, 2]).is_none());
    }
}
